...
// ── Init ─────────────────────────────────────────────────────────────────
/**
       * Fetches a FRESH profile rather than trusting the cached
       * APIHelper.getUserData() copy -- getting this wrong in either
       * direction is bad: a stale "false" lets a Google-only account
       * through without a password, a stale "true" incorrectly blocks
       * someone who already has one. Blocking modal has no close button
       * and no click-outside handler anywhere in its CSS or here.
       */
async function checkAndShowSetPasswordModal() {
  try {
    var res = await APIHelper.request(API_CONFIG.ENDPOINTS.USER.PROFILE);
    var profile = res && res.data ? res.data : res;
    if (profile && profile.hasPassword === false) {
      document.getElementById('setPasswordModal').classList.add('show');
    }
  } catch (e) {
    // Profile fetch failing shouldn't itself block the dashboard --
    // app-auth.js's own auth guard already handles a genuinely
    // expired session by redirecting to login.
  }
}

async function submitSetPassword() {
  var errEl = document.getElementById('spModalError');
  var newPassword = document.getElementById('spNewPassword').value;
  ...
  if (!/(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/.test(newPassword)) {
    errEl.textContent = 'Password must contain an uppercase letter, a lowercase letter, and a number.';
    errEl.style.display = 'block';
    return;
  }
  ...
  try {
    await APIHelper.request(API_CONFIG.ENDPOINTS.USER.SET_PASSWORD, {
      method: 'POST',
      body: JSON.stringify({ newPassword: newPassword })
    });
    // Refresh the cached user object so hasPassword is correct
    // immediately, without waiting for the next full login.
    var cached = APIHelper.getUserData();
    if (cached) {
      cached.hasPassword = true;
      APIHelper.saveUserData(cached);
    }
    document.getElementById('setPasswordModal').classList.remove('show');
  } catch (err) {
    errEl.textContent = (err && err.message) || 'Could not set password. Please try again.';
    errEl.style.display = 'block';
  } finally {
    btn.disabled = false;
...
document.addEventListener('DOMContentLoaded', async function () {
  await checkAndShowSetPasswordModal();
  ...
  var spSubmitBtn = document.getElementById('spSubmitBtn');
  if (spSubmitBtn) spSubmitBtn.addEventListener('click', submitSetPassword);
